...
export async function publishStories(
  ...
  const MIN_BODY_LENGTH = options?.minBodyLength ?? 500;
 ...
 * createVariantsOnDuplicate: when true and !forceUpsert, duplicates get name variants.
 * assignCoverImages: assigns random cover from public/story_cover/.
...
export async function saveSmartStoriesToFirestore(
  ...
  for (const raw of stories) {
    ...
    if (existingRef) {
      const { createdAt: _c, ...updateData } = doc as Record<string, unknown>;
