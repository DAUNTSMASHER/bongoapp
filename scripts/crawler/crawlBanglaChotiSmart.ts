...
/** Extract story links from listing/category page (article titles, not nav) */
export function extractStoryLinksFromListing(html: string, baseUrl: string): { url: string; title: string }[] {
  const $ = cheerio.load(html);
  const seen = new Set<string>();
  const links: { url: string; title: string }[] = [];

  $("article a[href], .entry-title a[href], .post-title a[href], .post a[href]").each((_, el) => {
  ...
  if (links.length === 0) {
    $("a[href]").each((_, el) => {
      const href = $(el).attr("href");
      if (!href || href.startsWith("#") || href.includes("/category/") || href.includes("/tag/")) return;
      try {
        const u = new URL(href, baseUrl);
        if (u.hostname !== new URL(baseUrl).hostname) return;
        const path = u.pathname.replace(/\/$/, "");
        const segments = path.split("/").filter(Boolean);
        if (segments.length < 2 || path.match(/\/page\/\d+/)) return;
        const abs = u.href;
        if (seen.has(abs)) return;
        const title = $(el).text().trim();
        if (!title || title.length < 5 || title.length > 120) return;
...
function extractStoryDetail(html: string): { headline: string; body: string; sourceTags?: string[] } | null {
  const $ = cheerio.load(html);
  const headline = $("h1").first().text().trim() || $(".entry-title").first().text().trim() || "";
  let body = "";
  let bestLen = 0;
