...
4. **Use PageSpeed Insights** on `https://bongochoti.com` (apex, not www) for Core Web Vitals before/after.
...
- **Fix SSL for www**: Ensure `www.bongochoti.com` has valid SSL. Vercel → Domains → add `www.bongochoti.com` and enable SSL.
- **Canonical URLs**: Already set on main pages ✅
- **Sitemap**: Already submitted at `https://bongochoti.com/sitemap.xml` ✅
