...
export default function AllStoriesPageClient() {
    ...
    getPublishedStoriesFromFirestore({ limitCount: 200 })
      .then((list) => {
        if (!cancelled) {
          ...
          list.forEach((s) => { if (!byId.has(s.id)) byId.set(s.id, s); });
      ...
      .catch(async () => {
        ...
        try {
          ...
          if (res.ok && Array.isArray(data?.stories) && data.stories.length > 0) {
            const mapped = data.stories.map((s: { createdAt?: string; updatedAt?: string; publishedAt?: string } & Record<string, unknown>) => {
            ...
            const byId = new Map<string, Story>();
            mapped.forEach((s: Story) => { if (!byId.has(s.id)) byId.set(s.id, s); });
  ...
  return (
    <ContentWrapper className="min-h-screen py-6 md:py-8">
      ...
      <PopAdPlacement placement="all-stories-page" />
