...
export default function SearchContent({ initialStories }: { initialStories: Story[] }) {
          ...
          {!q ? (
            ...
            <>
              {paginated.map((story, i) => (
                <StoryCard key={story.id} story={story} index={start + i} variant="list" onBeforeNavigate={adPopup?.showAdThenNavigate} />
              ))}
              <PaginationBar
